...
export const WalletProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const connect = useCallback(async (type: WalletType) => {
    ...
    try {
      ...
      if (msg.includes('not installed') || msg.includes('not available') || msg.includes('not found') || msg.includes('No module')) {
        setErrorType(AppErrorType.WALLET_NOT_FOUND);
        setError(`${type} wallet extension not found. Please install it first.`);
      } else if (msg.includes('User declined') || msg.includes('rejected') || msg.includes('cancel') || msg.includes('denied') || msg.includes('closed')) {
        setErrorType(AppErrorType.TRANSACTION_REJECTED);
        setError('Connection was cancelled or rejected.');
      } else if (type === 'xbull' && (msg.includes('timeout') || msg.includes('postMessage') || msg.includes('bridge'))) {
        setErrorType(AppErrorType.WALLET_NOT_FOUND);
        setError('Could not connect to xBull. Make sure the xBull popup was not blocked by your browser.');
      } else {
  ...
  return (
    <WalletContext.Provider value={{ wallet, loading, error, errorType, connect, disconnect }}>
      {children}
